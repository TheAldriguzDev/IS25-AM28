...
class GameModelTest {
    ...
    @Test
    public void test_game_model_hourglass() throws JsonProcessingException {
        // ========================================
        // NEW GAME HAS BEEN CREATED
        // ========================================
        assertInstanceOf(CreateGameState.class, model.getCurrentState());
        ...
        assertThrows(
                IllegalStateException.class,
                () -> model.selectTile("Player 2", 19),
                "The selected tile should not be available for a select"
        );

        // Deselect the tile
        tileState = model.deselectTile("Player 1", 19);
        ...
        // Select again the tile
        tileState = model.selectTile("Player 2", 19);
        ...
        assertFalse(timerDTO.getIsServerAction());
    ...
    @Test
    public void test_insufficient_player() throws Exception {
        // ========================================
        // NEW GAME HAS BEEN CREATED
        // ========================================
        assertInstanceOf(CreateGameState.class, model.getCurrentState());
        ...
        model.selectDeselectSubdeck("Player 2", 2, true);

        List<StateDTO> disconnectedStates;
        disconnectedStates = model.disconnectClient("Player 2");
        assertEquals(1, disconnectedStates.size());
        disconnectedStates = model.disconnectClient("Player 3");
        assertEquals(1, disconnectedStates.size());
        disconnectedStates = model.disconnectClient("Player 4");
        assertEquals(2, disconnectedStates.size());

        assertInstanceOf(InsufficientPlayerDTO.class, disconnectedStates.getLast());
        assertInstanceOf(InsufficientPlayerState.class, model.getCurrentState());

        model.reconnectClient("Player 2", null);
        model.reconnectClient("Player 3", null);
        model.reconnectClient("Player 4", null);

        assertDoesNotThrow(() -> model.selectDeselectSubdeck("Player 2", 2, true));

        assertThrows(
                Exception.class,
                () -> model.reconnectClient("Player 1", null),
                "The player nickname should not be in the disconnection list"
        );
